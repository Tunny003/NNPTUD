...
router.post('/',check_authorization(['mod']), async function(req, res, next) {
  try {
    let body = req.body;
...
router.put('/:id',check_authorization(['mod']), async function(req, res, next) {
  try {
    let id = req.params.id;
    let product = await productSchema.findById(id);
    if(product){
...
router.delete('/:id',check_authorization(['admin']), async function(req, res, next) {
  try {
    let id = req.params.id;
    let product = await productSchema.findById(id);
    if(product){
